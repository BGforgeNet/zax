...
describe("with the patch installed", () => {
  test("shows the version and says where it was read from", async () => {
  ...
  test("says ZAX does not manage the patch, so nobody waits for an update button", async () => {
    await plantLibrary("f2_res.dll", "4.1.8");
...
describe("with only the config file present", () => {
  test("says nothing reads these settings, rather than showing a blank version", () => {
    // The seeded install carries f2_res.ini and no f2_res.dll.
...
describe("either way", () => {
  test("offers no control at all, since there is nothing here to change", async () => {
    await plantLibrary("f2_res.dll", "4.1.8");
    const view = block();
